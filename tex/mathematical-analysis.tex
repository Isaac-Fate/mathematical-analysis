...
\begin{proof}
	...
	Let $\varepsilon > 0$ be arbitrary.
	Because $f$ is continuous uniformly on $Q$ (since $Q$ is compact),
	there exists $\delta > 0$ such that
	\begin{align*}
		\norm{(\tilde{x}, y) - (x, y_0)}
		\implies \abs{f(\tilde{x}, y) - f(x, y_0)}
		< \frac{\varepsilon}{\alpha(b) - \alpha(a)}
	\end{align*}
	for each $x \in [a, b]$.
	In particular, letting $\tilde{x} = x$, we have
	\begin{align*}
		\abs{y - y_0} < \delta
