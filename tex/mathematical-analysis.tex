...
\begin{proof}
	...
	By Lemma~\ref{lem:6}, we can extend $\phi$ 
	...
	Note that $\phi^\prime(x)$ exists for each $x$ in $[c,d]$ 
	($x$ is an interior point of $[c-\delta, d+\delta]$)
	and that $F$ is differentiable at $\phi(x)$
	($\phi(x)$ is an interior point in $\phi([c-\delta, d+\delta])$)
