...
\begin{example}
    ...
    \begin{align*}
        ...
        -1 \leq x \leq 1
    \end{align*}
    And let $f$ and $g$ be any two functions defined on $[-1, 1]$ that are continuous from the left at $x=0$. By Theorem~\ref{thm:24} and Theorem~\ref{thm:40}, we know both functions are integrable w.r.t. $\alpha$ on $[-1, x]$ if $-1 < x \leq 1$ with 
    \begin{align*}
