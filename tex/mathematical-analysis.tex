...
If we consider the derivative of $f$, $f^\prime$ as a function, then $f^\prime(x)$ may tend to infinity as $x$ approaches an endpoint. Therefore, we also need to define \textbf{infinite derivatives}\index{infinite derivative}.
...
It is very difficult to imagine the scenario that $f^\prime_{+}(c) = \infty$ provided $f$ is continuous at $c$. But we indeed have such an example (Example~\ref{eg:1}).
