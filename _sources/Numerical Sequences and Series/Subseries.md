...
```{math}
\begin{align*}
a_1, a_2, \ldots, a_n, \ldots\end{align*}
```
...
```{math}
\begin{align*}
s_1 = a_1 + a_3 + \cdots + a_{2k-1} + \cdots\end{align*}
```
...
```{math}
\begin{align*}
s_2 = a_2 + a_4 + \cdots + a_{2k} + \cdots\end{align*}
```
...
```{math}
\begin{align*}
a_{k_1}, a_{k_2}, \ldots a_{k_n}, \ldots\end{align*}
```
...
```{math}
\begin{align*}
a_{g(k_1)}, a_{g(k_2)}, \ldots a_{g(k_n)}, \ldots\end{align*}
```
...
```{math}
\begin{align*}\sum_{n=1}^\infty i^n
= i + (-1) + (-i) + 1
+ i + (-1) + (-i) + 1 + \cdots\end{align*}
```
...
```{math}
\begin{align*}\sum a_{f(n)} = (-i) + i + (-i) + i + \cdots\end{align*}
```
...
```{math}
:label: eq:105
\begin{align}\abs{\sum_{n=1}^\infty b_n}\leq\sum_{n=1}^\infty\abs{b_n}\leq\sum_{n=1}^\infty\abs{a_n}\end{align}
```
...
```{math}
\begin{align*}\abs{\sum_{k=1}^n b_k}\leq\sum_{k=1}^n \abs{b_k}\leq\sum_{k=1}^N \abs{a_k}\leq\sum_{k=1}^\infty\abs{a_k}\end{align*}
```

Since

```{math}
\begin{align*}\abs{\sum_{k=1}^n b_k}\leq\sum_{k=1}^n \abs{b_k}\leq\sum_{k=1}^\infty\abs{a_k}\end{align*}
```
...
```{math}
\begin{align*}\sum_{n=1}^\infty a_{f_1(n)}, \sum_{n=1}^\infty a_{f_2(n)}, \ldots,
\sum_{n=1}^\infty a_{f_k(n)}, \ldots\end{align*}
```
...
- 1️⃣ $f_k$ is injective,
- 2️⃣ the range $f_k(\N^\ast)$ is a subset of $\N^\ast$, and
- 3️⃣ the collection of all function ranges form a partition of $\N^\ast$, that is, $\biguplus_{k=1}^\infty f_k(\N^\ast) = \N^\ast$.
...
\quad
n \in\N^\ast, \;
k \in\N^\ast\end{align*}
```

Then
- 1️⃣ each $\sum_n b_k(n)$ is an absolutely convergent subseries of $\sum a_n$, and
- 2️⃣ if we let $s_k = \sum_{n=1}^\infty b_k(n)$, then $\sum s_k$ converges absolutely to the same sum as $\sum a_n$, that is, $\sum_{k=1}^\infty s_k = \sum_{n=1}^\infty a_n$.
````
...
```{math}
\begin{align*}\sum_{k=1}^p \abs{s_k}&= \abs{s_1} + \cdots + \abs{s_p}\\&= \abs{ \sum_{n=1}^\infty b_1(n) } + \cdots
+ \abs{ \sum_{n=1}^\infty b_p(n) }\\&\leq\sum_{n=1}^\infty\abs{b_1(n)} + \cdots
+ \sum_{n=1}^\infty\abs{b_p(n)}\\&= \sum_{n=1}^\infty( \abs{b_1(n)} + \cdots
+ \abs{b_p(n)})
\end{align*}
```
...
```{math}
\begin{align*}\sum_{n=1}^q ( \abs{b_1(n)} + \cdots
+ \abs{b_p(n)})
\leq\sum_{n=1}^\infty\abs{a_n}\end{align*}
```
...
```{math}
\begin{align*}\sum_{k=1}^p \abs{s_k}\leq\sum_{n=1}^\infty( \abs{b_1(n)} + \cdots
+ \abs{b_p(n)})
\leq\sum_{n=1}^\infty\abs{a_n}\end{align*}
```
...
```{math}
:label: eq:107
\begin{align}\abs{\sum_{n=1}^q a_n - s} < \varepsilon / 2
\quad\forall q \geq N
\end{align}
```
...
```{math}
:label: eq:106
\begin{align}\abs{\sum_{k=1}^p \sum_{n=1}^q b_k(n) - s} < \varepsilon / 2
...
```{math}
\begin{align*}\abs{\sum_{k=1}^p s_k - s}
...
```{math}
\begin{align*}\sum_{k=1}^p \sum_{n=1}^q b_k(n)
= \sum_{k=1}^p \sum_{n=1}^q a_{f_k(n)}\end{align*}
```
