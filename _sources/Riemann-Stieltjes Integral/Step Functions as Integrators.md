...
```{math}
\begin{align*}\alpha(x) = \begin{cases}
\alpha(a) & a \leq x < c \\
\alpha(b) & c < x \leq b
\end{cases}\end{align*}
```
...
- 1️⃣ at least one of $f$ and $\alpha$ is continuous from the left at $c$, and
- 2️⃣ at least one of $f$ and $\alpha$ is continuous from the right at $c$.
...
```{math}
\begin{align*}
S(P,f,\alpha) = f(c) [\alpha(b) - \alpha(a)]\end{align*}
```
...
+ f(c+\delta_2) [\alpha(b) - \alpha(c)]\end{align*}
```
...
+ \abs{f(c+\delta_2) - f(c)}\abs{\alpha(b) - \alpha(c)}\end{multline}
```
...
- 1️⃣ $\alpha(x)$ is continuous at $x=c$.
- 2️⃣ $\alpha(x)$ is continuous from the right at $x=c$, and $f(x)$ is continuous from the left at $x=c$.
- 3️⃣ $\alpha(x)$ is continuous from the left at $x=c$, and $f(x)$ is continuous from the right at $x=c$.
- 4️⃣ $f(x)$ is continuous at $x=c$.(Proof of 1) If $\alpha(x)$ is continuous at $x=c$, then $\alpha$ is a constant function. In this case, $f$ is certainly integrable with respect to $\alpha$ with $\int_{a}^{b} f \; \mathrm{d}\alpha = 0$. And {eq}`eq:40` holds since its right-hand side is also zero for $\alpha(a) = \alpha(b)$.
...
S(P,f,\alpha)
- f(c) [\alpha(b) - \alpha(a)]
}\leq\abs{f(c-\delta_1) - f(c)}\abs{\alpha(c) - \alpha(a)}\end{align}
```
...
```{math}
\begin{align*}\lim_{x \to 0}\frac{\sin (x)}{x} = 1
\end{align*}
```
...
```{math}
\begin{align*}\int_{-3\pi}^{3\pi}f \;\mathrm{d}\alpha
...
```{math}
\begin{align*}\abs{S(P,f,\alpha) - A} + \abs{S^\prime(P,f,\alpha) - A}\geq\abs{S(P,f,\alpha) - S^\prime(P,f,\alpha)}
> 3 > 2\varepsilon\end{align*}
```
...
[Hint: You may find the theorem of integration by parts ({prf:ref}`thm:20`) helpful.]
````
