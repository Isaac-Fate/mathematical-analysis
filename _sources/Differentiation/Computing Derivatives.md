...
- 1️⃣ $(f \pm g)^\prime(c) = f^\prime(c) \pm g^\prime(c)$,
- 2️⃣ $(f g)^\prime(c) = f^\prime(c) g(c) + f(c) g^\prime(c)$,
- 3️⃣ $(f / g)^\prime(c) = \frac{f^\prime(c) g(c) - f(c) g^\prime(c)}{ g^2(c) }$, provided $g(c) \neq 0$.
````
