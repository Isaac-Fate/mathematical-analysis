...
defined in  {ref}`Exercise 3<ex:1>` . Show that  $f(0)$  is not a local extremum. Recall we have shown that  $f^\prime(0) = 0$ . This is another example that the converse of  {prf:ref}`thm:5`  does not hold.
````
